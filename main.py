...
#The script download the html files on the Internet. This section checks the network connection availability with request for gooogle.com
...
#The script works with html tags, so need to check the input is valid html docs.The script search text/html string in htmls's header.
...
#This section parse the html with BeautifulSoup and find the links tag that have a type attribute with rss or atom feeds value.
...
json = json.dumps(dic)
print(f"Job completed, the following json has been made: {json}")
